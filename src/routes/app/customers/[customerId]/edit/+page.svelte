<script lang="ts">
  ...
  // $state variables for form fields, initialized from form?.fields (if validation error) or data.customer
  let name = $state(form?.fields?.name ?? data.customer.name ?? '');
  let email = $state(form?.fields?.email ?? data.customer.email ?? '');
  let phone = $state(form?.fields?.phone ?? data.customer.phone ?? '');
  let address_line1 = $state(form?.fields?.address_line1 ?? data.customer.address_line1 ?? '');
  let address_line2 = $state(form?.fields?.address_line2 ?? data.customer.address_line2 ?? '');
  let city = $state(form?.fields?.city ?? data.customer.city ?? '');
  let postal_code = $state(form?.fields?.postal_code ?? data.customer.postal_code ?? '');
  let country = $state(form?.fields?.country ?? data.customer.country ?? '');
  ...
  // Add more derived errors for other fields if specific messages are provided by Zod/server
...
<div class="container mx-auto p-4">
  <h1 class="text-2xl font-semibold mb-6">Edit Customer: <span class="font-normal italic">{data.customer.name}</span></h1>
  ...
  <form method="POST" class="space-y-6 bg-white shadow-xl rounded-lg p-8" use:enhance>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div>
        ...
        {#if form?.errors?.phone}<p class="mt-1 text-xs text-red-600">{form.errors.phone[0]}</p>{/if}
      </div>
    </div>
    ...
    <div>
      ...
      <input type="text" name="address_line1" id="address_line1" bind:value={address_line1}
             class="input mt-1 block w-full" />
      {#if form?.errors?.address_line1}<p class="mt-1 text-xs text-red-600">{form.errors.address_line1[0]}</p>{/if}
    </div>
    <div>
      ...
      <input type="text" name="address_line2" id="address_line2" bind:value={address_line2}
             class="input mt-1 block w-full" />
      {#if form?.errors?.address_line2}<p class="mt-1 text-xs text-red-600">{form.errors.address_line2[0]}</p>{/if}
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div>
        ...
        <input type="text" name="city" id="city" bind:value={city}
               class="input mt-1 block w-full" />
        {#if form?.errors?.city}<p class="mt-1 text-xs text-red-600">{form.errors.city[0]}</p>{/if}
      </div>
      <div>
        <label for="postal_code" class="block text-sm font-medium text-gray-700">Postal Code</label>
        <input type="text" name="postal_code" id="postal_code" bind:value={postal_code}
               class="input mt-1 block w-full" />
        {#if form?.errors?.postal_code}<p class="mt-1 text-xs text-red-600">{form.errors.postal_code[0]}</p>{/if}
      </div>
      <div>
        <label for="country" class="block text-sm font-medium text-gray-700">Country</label>
        <input type="text" name="country" id="country" bind:value={country}
               class="input mt-1 block w-full" />
        {#if form?.errors?.country}<p class="mt-1 text-xs text-red-600">{form.errors.country[0]}</p>{/if}
      </div>
    </div>
...
<style>
  ...
  .font-semibold { font-weight: 600; } .font-medium { font-weight: 500; } .font-normal { font-weight: normal; } .italic { font-style: italic; }
