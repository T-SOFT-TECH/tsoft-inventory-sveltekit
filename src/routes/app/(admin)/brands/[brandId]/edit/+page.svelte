<script>
  ...
  export let data; // Contains brand from load function

  /** @type {import('./$types').ActionData} */
  export let form; // Contains data from form submission if it failed

  // Initialize form field values:
  // 1. Use `form` data if available (after a failed submission attempt)
  // 2. Otherwise, use `data.brand` data (on initial page load)
  // 3. Fallback to empty string for optional fields like logo_url if not present in data.brand
  let currentName = form?.name ?? data.brand.name;
  let currentLogoUrl = form?.logo_url ?? data.brand.logo_url ?? '';

  // $: console.log('Edit page - Form data:', form);
  // $: console.log('Edit page - Initial brand data:', data.brand);
</script>
...
<div class="container mx-auto p-4">
  ...
  <form method="POST" class="space-y-6 bg-white shadow-md rounded-lg p-8">
    ...
    <div>
      ...
      {#if form?.missingName}
        <p class="mt-2 text-sm text-red-600">{form.message || 'Brand name is required.'}</p>
      {:else if form?.duplicateName}
        <p class="mt-2 text-sm text-red-600">{form.message || 'A brand with this name already exists.'}</p>
      {/if}
    </div>

    <div>
      <label for="logo_url" class="block text-sm font-medium text-gray-700">Logo URL (Optional)</label>
      <input type="url" name="logo_url" id="logo_url" placeholder="https://example.com/logo.png" bind:value={currentLogoUrl}
      ...
      {#if form?.invalidUrl}
        <p class="mt-2 text-sm text-red-600">{form.message || 'The provided Logo URL is not valid.'}</p>
      {/if}
    </div>
